...
download_args:
  ...
  live:
    ...
    # 弹幕描边宽度
    outlinesize: 1.0
    # 短时间重复弹幕分级样式：相同内容连续出现时，按累计次数改变颜色并升级为滚动热点
    # 连续重复字符及短符号反应会自动归并，例如“？”与“？？？”、“😂”与“😂😂”
    # 普通句子不会仅因包含相同符号或文字而合并
    repeat_danmaku:
      # 是否启用；默认关闭，关闭后所有弹幕仍按原有样式滚动
      enabled: False
      # 会话超时（秒）：相同内容超过该时间未再次出现，则会话结束并从1重新计数
      # 热点在途期间不会超时；完全离屏后才开始计时。弹幕文件分段时会结束当前会话
      session_timeout: 8
      # 单轮热点从右侧完全画外滚动到左侧完全画外的时长（秒）
      hot_duration: 8
      ...
      pre_hot_max_width: 0  # 0表示滚动时保留完整内容
      hot_min_font_scale: 1.0
      hot_ellipsis: False  # 滚动热点推荐关闭，不省略长文本
      hot_max_visible: 0  # 同时在途上限；0表示由可用普通弹幕轨道决定
      # 热点参与普通滚动碰撞计算时的高度倍率
      hot_line_spacing: 1.25
      # 热点与普通弹幕使用相同的滚动尾随逻辑；间距足够后普通弹幕可继续进入同一轨道
